using WatchStore.Web.Libraries;
using WatchStore.Web.Models;
using WatchStore.Web.Models.Payment;
...
namespace WatchStore.Web.Services
{
    public class VnPayService : IVnPayService
    {
        ...
        public string CreatePaymentUrl(Order model, HttpContext context)
        {
            ...
            pay.AddRequestData("vnp_Amount", ((long)model.Total * 100).ToString());
            pay.AddRequestData("vnp_CreateDate", timeNow.ToString("yyyyMMddHHmmss"));
            pay.AddRequestData("vnp_CurrCode", _configuration["Vnpay:CurrCode"]);
            pay.AddRequestData("vnp_IpAddr", pay.GetIpAddress(context));
            pay.AddRequestData("vnp_Locale", _configuration["Vnpay:Locale"]);
            pay.AddRequestData("vnp_OrderInfo", $"{model.FullName}");
            pay.AddRequestData("vnp_OrderType", model.Address);
